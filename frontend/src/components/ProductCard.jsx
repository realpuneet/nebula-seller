...
function ProductCard({ id, title, description, images, price, stock }) {
  // const {id, title, description, images, price, stock} = product;
  ...
  return (
    <div className="relative bg-white shadow-lg rounded-xl overflow-hidden flex flex-col transition hover:scale-[1.02] duration-150 w-full max-w-sm">
      ...
      {/* Product Images */}
      {images && images && (
        <img src={images} alt={title} className="h-48 w-full object-cover" />
      )}
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mt-auto flex justify-between items-center pt-4">
          ...
          <button
            ...
            className="px-4 py-2 bg-blue-500 text-white rounded-md font-bold hover:bg-blue-600 ml-2 transition"
          >
            View
          </button>
        </div>
